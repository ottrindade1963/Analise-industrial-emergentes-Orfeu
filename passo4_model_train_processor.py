...
class ModelTrainer:
    ...
    def prepare_data(self):
        ...
        try:
            ...
            # Preencher NaNs nas features com 0 para evitar erro "Input contains NaN"
    ...
    def train_all(self):
        ...
        for model_name in config.MODELS_TO_TRAIN:
            ...
            if model_name == 'RandomForest': self.train_random_forest()
            elif model_name == 'XGBoost': self.train_xgboost()
            elif model_name == 'SARIMAX': self.train_sarimax()
            elif model_name == 'LSTM': self.train_lstm()
            elif model_name == 'TFT': self.train_tft()
            ...
            if model_name in self.models:
                ...
                try:
                    if model_name == 'SARIMAX':
                        exog_cols = self.X_train.columns[:5]
                        preds = model.predict(start=0, end=len(self.X_train)-1, exog=self.X_train[exog_cols].values)
                    else:
        ...
        self.save_models()
        return self.models, self.histories, training_metrics
...
def run_training_for_all():
    ...
    for dataset in config.DATASETS:
        for strategy in config.STRATEGIES:
            ...
            if os.path.exists(filepath):
                ...
                _, _, metrics = trainer.train_all()
                
                # Salvar métricas no log global
    ...
    print(f"\nLogs de treinamento salvos em: {log_path}")
