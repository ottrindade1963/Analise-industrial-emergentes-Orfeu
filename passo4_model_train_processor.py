...
class ModelTrainer:
    def __init__(self, df, dataset_name, strategy_name):
        ...
        # Identificar colunas de país e ano
        country_col = 'pais' if 'pais' in self.df.columns else ('country' if 'country' in self.df.columns else None)
        year_col = 'ano' if 'ano' in self.df.columns else ('year' if 'year' in self.df.columns else None)
        
        if country_col and year_col:
            self.df = self.df.sort_values(by=[country_col, year_col])

    def prepare_data(self):
        """Divide os dados em Treino, Validação e Teste (Walk-Forward Temporal por Ano)."""
        # Identificar a coluna de ano
        year_col = 'year' if 'year' in self.df.columns else 'ano'
        
        # Criar máscaras booleanas baseadas nos anos de corte
        train_mask = self.df[year_col] <= config.TRAIN_END_YEAR
        val_mask = (self.df[year_col] > config.TRAIN_END_YEAR) & (self.df[year_col] <= config.VAL_END_YEAR)
        test_mask = self.df[year_col] > config.VAL_END_YEAR
        
        cols_to_drop = ['country', 'year', 'ano', 'pais']
        features = [c for c in self.df.columns if c not in cols_to_drop and c != config.TARGET_VAR]
        
        # Dividir X e y usando as máscaras
        self.X_train = self.df.loc[train_mask, features]
        self.y_train = self.df.loc[train_mask, config.TARGET_VAR]
        
        self.X_val = self.df.loc[val_mask, features]
        self.y_val = self.df.loc[val_mask, config.TARGET_VAR]
        
        self.X_test = self.df.loc[test_mask, features]
        self.y_test = self.df.loc[test_mask, config.TARGET_VAR]
