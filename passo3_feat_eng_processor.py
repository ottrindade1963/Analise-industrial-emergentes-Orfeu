...
class FeatureEngineer:
    def __init__(self, df, dataset_name):
        ...
        if not self.quant_vars:
            ...
            numeric_cols = [c for c in numeric_cols if c not in self.qual_vars and c != config.TARGET_VAR and c not in ['year', 'ano']]
...
def load_and_process_datasets():
    ...
    for name, filename in config.DATASETS.items():
        ...
        if os.path.exists(filepath):
            ...
            # Criar dados dummy para demonstração se o ficheiro não existir
            print(f"Criando dados dummy para {name}...")
            df_dummy = create_dummy_data(name)
            engineer = FeatureEngineer(df_dummy, name)
            strategies_dfs = engineer.process_all_strategies()
            ...
            for strategy, df_strat in strategies_dfs.items():
                ...
                out_filepath = os.path.join(config.OUTPUT_DIR, out_filename)
                df_strat.to_csv(out_filepath, index=False)
                print(f"  -> Salvo dummy: {out_filename} (Shape: {df_strat.shape})")
                
            results[name] = strategies_dfs
            
    return results

def create_dummy_data(dataset_name):
    """Cria dados dummy para teste caso os ficheiros reais não existam."""
    np.random.seed(42)
    n_samples = 100
    
    data = {
        'country': ['Country_' + str(i%10) for i in range(n_samples)],
        'year': [2000 + (i//10) for i in range(n_samples)],
        config.TARGET_VAR: np.random.uniform(10, 30, n_samples),
        'fbc_percent_pib': np.random.uniform(15, 35, n_samples),
        'ide_percent_pib': np.random.uniform(1, 10, n_samples),
        'comercio_percent_pib': np.random.uniform(20, 80, n_samples)
    }
    
    # Adicionar variáveis qualitativas apenas para os datasets agregados
    if dataset_name != 'nao_agregado':
        for var in config.QUALITATIVE_VARS:
            data[var] = np.random.uniform(-2.5, 2.5, n_samples)
